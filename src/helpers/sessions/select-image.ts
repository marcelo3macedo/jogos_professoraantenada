function handleSelectImage(type: string, s: any, content: any) {
  const { word, slug } = s || {};
...
function generateOptions(words: any, correctWord: string, count: number) {
  const correctWordObj = words.find(
    (word: any) => word.word.toLowerCase() === correctWord.toLowerCase(),
  );
  if (!correctWordObj) {
